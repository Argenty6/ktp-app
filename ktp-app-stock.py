...
class SpotObservation(db.Model):
    ...
    spot_id = db.Column(
        db.String(50), db.ForeignKey("kite_spots.spot_id"), nullable=False
    )
...
class DailyMaxWindSpeed(db.Model):
    ...
    spot_id = db.Column(
        db.Integer, db.ForeignKey("spot.id")
    )  # Assurez-vous que cette définition correspond à votre modèle
    date_obs = db.Column(db.Date, nullable=False)
...
def spot_details(spot_id):
    ...
    direction_distribution_raw = (
        db.session.query(
            SpotObservation.direction_vent % 360, func.count().label("direction_count")
        )
        .filter(
    ...
    start_index = next(
        (i for i, pair in enumerate(direction_distribution_raw) if pair[0] >= 180), None
    )
    if start_index is not None:
        direction_distribution_reordered = (
            direction_distribution_raw[start_index:]
            + direction_distribution_raw[:start_index]
        )
    else:
    ...
    colors = [
        "rgba(255, 0, 0, 0.6)" if value > 25 else "rgba(0, 0, 255, 0.6)"
        for value in direction_data
    ]
    ...
    response_data = {
        ...
        "datasets": [
            {
                "label": "12-16 knots",
                "data": [d["pct_12_16"] for d in data_list],
                "backgroundColor": "rgba(255, 99, 132, 0.5)",
            },
            {
                "label": "17_21 knots",
                "data": [d["pct_17_21"] for d in data_list],
                "backgroundColor": "rgba(0, 99, 132, 0.5)",
            },
            {
                "label": "22_27 knots",
                "data": [d["pct_22_27"] for d in data_list],
                "backgroundColor": "rgba(140, 99, 132, 0.5)",
            },
            {
                "label": "28_33 knots",
                "data": [d["pct_28_33"] for d in data_list],
                "backgroundColor": "rgba(190, 99, 132, 0.5)",
            },
            {
                "label": "34_40 knots",
                "data": [d["pct_34_40"] for d in data_list],
                "backgroundColor": "rgba(255, 80, 132, 0.5)",
            },
            {
                "label": "41_90 knots",
                "data": [d["pct_41_90"] for d in data_list],
...
class WindStatistics:
    def __init__(self, localization: str, db_path: str):
        self.localization = localization
        self.db_path = db_path

    def show_statistics_at_given_period(self, period: str) -> list:
        stats = self.db.filter(month=period)
